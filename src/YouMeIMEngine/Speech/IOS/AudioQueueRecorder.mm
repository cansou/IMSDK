...
-(AudioErrorCode) StartRecord:(NSString*)path serial:(unsigned long long)serial
{
    ...
    for (int i = 0; i < BUFFER_NUMBER; ++i)
    {
        *((int*)_m_aqData.buffers[i]->mUserData) = 1;
        ...
        if (status != noErr)
        {
            ...
            _m_aqData.status = AUDIOSTATUS_IDEL;  // add
    ...
    if (status != noErr)
    {
        ...
        _m_aqData.status = AUDIOSTATUS_IDEL;
        YouMe_LOG_Error(__XT("AudioQueueStart error(%ld) "),(long)status);
        AudioQueueFlush(_m_aqData.queue);
        AudioQueueReset(_m_aqData.queue);
...
-(void) ResetAudioCategory
{
    ...
    AVAudioSession *audioSession=[AVAudioSession sharedInstance];
    if (audioSession && ![[audioSession category] isEqualToString:_m_audioSessionCategory])
    {
