...
//        [audioSession setCategory:AVAudioSessionCategoryPlayAndRecord withOptions:AVAudioSessionCategoryOptionDefaultToSpeaker error:nil];
        ...
        if( ![audioSessionCategory isEqualToString:AVAudioSessionCategoryPlayAndRecord] ){
            if (![AudioRecord isHeadsetPluggedIn]) {
    ...
    NSLog(@"record");
      //异步时需要
//    if(_forceStopRecording && bRet){
//        [self CancelRecord];
//        return false;
//    }
    if (bRet == NO) {
        YouMe_LOG_Error(__XT("record audio error"));
        if (!keepRecordModel && audioSession && audioSessionCategory != nil) {
            [audioSession setCategory:audioSessionCategory withOptions:AVAudioSessionCategoryOptionDefaultToSpeaker error:nil];
        }
        return false;
    }
...
+ (BOOL)isHeadsetPluggedIn {
    ...
    for (AVAudioSessionPortDescription* desc in [route outputs]) {
        NSLog(@"%@",[desc portType]);
        if ([[desc portType] isEqualToString:AVAudioSessionPortHeadphones] || [[desc portType] isEqualToString:AVAudioSessionPortBluetoothHFP] || [[desc portType] isEqualToString:AVAudioSessionPortBluetoothLE])
            return YES;
    }
    return NO;
}
