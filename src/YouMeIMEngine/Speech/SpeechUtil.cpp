...
bool SpeechUtil::PcmToWav(const XString& srcPath, unsigned short sampleBitSize, unsigned short channels, unsigned long sampleFrequency, int format, const XString desPath)
{
	...
	headInfo.riffID[2] = 'F';
	headInfo.riffID[3] = 'F';
	headInfo.fileSize = (int)(srcFileSize + sizeof(WAVHeadInfo) - 8);
	...
	headInfo.sampleFrequency = (unsigned int)sampleFrequency;
	...
	headInfo.dataID[2] = 't';
	headInfo.dataID[3] = 'a';
	headInfo.dataChunkSize = (unsigned int )srcFileSize;
	...
	{
		char szBuf[16384];
		size_t stRead = fread(szBuf, 1, 16384, pFileSrc);
		if (stRead > 0)
		{
...
int SpeechUtil::GetWavTime(const XString& path)
{
	...
	fread(&byteRate, sizeof(unsigned int), 1, pFile);
	if (byteRate == 0)
	{
		fclose(pFile);
		return 0;
	...
	fread(&dataChunkSize, sizeof(unsigned int), 1, pFile);
    ...
    */
    ...
    long dataChunkSize = ftell(pFile);
    if (dataChunkSize <= 46)
    {
        fclose(pFile);
        return 0;
	...
	int nTime = round( dataChunkSize * 1.0 / byteRate );
...
int SpeechUtil::GetAmrnbTime(const XString& path)
{
	...
	int nTime = ((int)fileSize - headSize + 1) / frameSize / 50;
	return nTime == 0 ? 1 : nTime;
}
...
FileFormat SpeechUtil::GetFileFormat(const XString& strFilePath)
{
	...
	FileFormat format = FileFormat_Unknow;
	char header[8] = { 0 };
	file.Read((byte*)header, sizeof(header));
	if (memcmp(header, "RIFF", 4) == 0)
	{
		format = FileFormat_WAV;
	}
	if (memcmp(header, "OPUS", 4) == 0)
	{
		format = FileFormat_OPUS;
	}
	if (memcmp(header, "#!AMR\n", 6) == 0)
	{
		format = FileFormat_AMR;
	}
